...
var StreamedOutgoingFrame = require('./StreamedOutgoingFrame.js');
var StreamedIncomingFrame = require('./StreamedIncomingFrame.js');
...
function WebSocketBase() {
	...
	this._incomingStream = null; // StreamedIncomingFrame object for the current message
...
WebSocketBase.prototype.createMessageStream = function(type) {
	var frame = new StreamedOutgoingFrame(this, type);
...
WebSocketBase.prototype._handleFrame = function(frame) {
	...
	if (!frame.FIN && !this._incomingStream) {
		...
		this._incomingStream = new StreamedIncomingFrame(frame, dispatch);
...
WebSocketBase.prototype._processQueue = function() {
	...
	while (frames.length > 0) {
		...
		if (frame instanceof StreamedOutgoingFrame) {
			if (!frame.started) {
				this.emit('debug', "Starting StreamedOutgoingFrame");
				frame._start();
			}
